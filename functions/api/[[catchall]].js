// ═══════════════════════════════════════════════════════════════════
// MANIT SOLUTIONS Payroll — Cloudflare Pages API (D1-backed)
// ═══════════════════════════════════════════════════════════════════
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (result && result.body) {
      // Already a Response-like object
...
async function getJWTSecret(env) {
  // Try env secret first, then D1, then generate and store
  ...
  if (path === 'settings') {
    ...
    if (method === 'PUT') {
      ...
      for (const [key, value] of Object.entries(body)) {
        if (key === 'jwt_secret') continue; // never expose
